...
Sprite::Sprite(void)
{
	// ComPtr 는 기본 생성 시 nullptr 로 초기화됨 (별도 처리 불필요)
}
...
bool Sprite::Create(const char* filename, bool bUseTransparency, D3DCOLOR TransparencyColor)
{
	...
	hr = D3DXCreateTextureFromFileEx(dv_font.Device9 ,
		...
		Texture.ReleaseAndGetAddressOf() ) ;   // 재생성 시 기존 텍스처 자동 Release

	// 만약 경로에 이미지가 없어서 로딩에 실패했다면?
	if (FAILED(hr)) {
		Texture.Reset(); // 텍스처를 비우고
...
{   
	if (!Texture) return;// sx -1 : 좌우반전, sy -1 = 상하반전
    RECT Rect ;                                                           // sx 0 : 없어짐, 1 : 그대로, 2 : x축으로 2배 확대
