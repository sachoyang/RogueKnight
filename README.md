...
유명 인디 게임 *'Hollow Knight'*의 모작 프로젝트로, **절차적 맵 생성(Procedural Generation)**을 통한 로그라이크 요소와 **유한 상태 머신(FSM)** 기반의 전략적인 보스 전투, 그리고 **A\* 길찾기** 기반의 지능형 적 추격 AI를 구현하는 데 중점을 두었습니다.
...
* **AI / Pathfinding:** A\* Search (연속 물리 월드의 격자 래스터화)
...
### D. 🧭 A\* 길찾기 기반 적 추격 AI (A\* Pathfinding)

적(`Enemy`)이 벽과 지형을 우회하여 플레이어를 지능적으로 추격하도록 **A\* 최단 경로 탐색**을 구현했습니다 (`Pathfinding.cpp`, `Enemy.cpp`).

1.  **월드 래스터화 (Grid Rasterization):** 이 게임은 격자가 아닌 **연속 물리 월드**이므로, 맵을 `CELL_SIZE`(40px) 간격의 2차원 격자(`NavGrid`)로 변환합니다. 각 셀이 충돌 벽(`coll.m_Walls`)과 겹치면 이동 불가(`walkable = false`)로 마킹합니다.
2.  **동적 격자 재구성:** 맵이 이동/변경될 때마다 `navGrid.Build()`를 호출하여, 절차적으로 생성된 새 맵 구조에 맞춰 격자를 다시 구성합니다.
3.  **휴리스틱(Heuristic):** 8방향 이동을 지원하며, **대각 14 / 직선 10**의 옥타일 거리(Octile Distance)를 추정 비용(`hCost`)으로 사용해 `F = G + H`가 최소인 노드를 우선 탐색합니다.
4.  **성능 최적화 (Repath Throttling):** 매 프레임 A\*를 돌리는 부담을 줄이기 위해, 추격 중에는 **400ms 주기**로만 경로를 재계산합니다(경로가 비면 즉시 계산). 계산된 웨이포인트는 `PickWaypoint`로 순차 추종합니다.
5.  **디버그 시각화:** 디버그 모드(`/`)에서 열림/닫힘 리스트, 이웃, 확정 경로, 시작/타겟 셀을 **상태별 색상**으로 오버레이하여 탐색 과정을 눈으로 확인할 수 있습니다. (적이 여러 마리일 때 경로마다 다른 색상 부여)
